...
function AdminPage() {
  ...
  useEffect(() => {
    ...
    axios.get("http://localhost:5000/api/attendance/dates-in-month", { params: { month: monthStr } })
      .then(res => setAttendanceMarkedDates(res.data || []));
  }, [attendanceMonth]);

  useEffect(() => {
    setLoading(true);
    axios.get("http://localhost:5000/api/attendance/all", { params: { date: selectedDate } })
  ...
  useEffect(() => {
    ...
    axios.get("http://localhost:5000/api/tasks/dates-in-month", { params: { month: monthStr } })
      .then(res => setTasksMarkedDates(res.data || []));
  }, [taskMonth]);

  useEffect(() => {
    setLoading(true);
    axios.get("http://localhost:5000/api/tasks/all", { params: { date: selectedTaskDate } })
  ...
  const handleDuplicateTasks = async () => {
    ...
    try {
      await axios.post("http://localhost:5000/api/tasks/duplicate-to-end-of-month", {
        date: selectedTaskDate
      });
      // reload task, reload ngày có task của tháng
      const res = await axios.get("http://localhost:5000/api/tasks/all", { params: { date: selectedTaskDate } });
      setTasks(res.data);
      const d = new Date(selectedTaskDate);
      const monthStr = `${d.getFullYear()}-${(d.getMonth() + 1).toString().padStart(2, "0")}`;
      const dres = await axios.get("http://localhost:5000/api/tasks/dates-in-month", { params: { month: monthStr } });
